...
// const int motorPinThree = 5;
// const int motorPinFour = 6;
...
{ 
    ...
    for(int i=0;i<128;i++)
    {
        a=analogRead(A4)-500;     //rough zero shift
    ...
    if(sum2-sum1>5){
        ...
        a2=micros();
        k=chord;
        if(k==0){Serial.print('C');Serial.println(chord_out);Serial.println(k) }
...
// EasyFFT code optimised for 128 sample size to reduce memory consumtion
